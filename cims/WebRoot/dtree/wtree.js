...
// Node object
function Node(id, pid, name, url, title, target, checkboxname, checkboxvalue, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
function dTree(objName,iconPath) {
	...
	this.idxNode = new Object();
};

// Adds a new node to the node array
dTree.prototype.add = function(id, pid, name, url, title, target, checkboxname, checkboxvalue, icon, iconOpen, open) {
	var idx = this.aNodes.length;
	var node = new Node(id, pid, name, url, title, target, checkboxname, checkboxvalue,icon, iconOpen, open);
	node.idx = idx;
	this.aNodes[idx] = node;
	
	// link parent and children.
	this.idxNode[id] = node;
};
...
dTree.prototype.toString = function() {
	...
	for (var i = 0; i < this.aNodes.length; i++) {
		...
		node.parent = this.idxNode[node.pid];
...
//initAllCheckBox,selectedIds 用$分隔
//added by liuyong 2011-07-12
dTree.prototype.initCheckBoxNodes=function(selectedIds){
...
dTree.prototype.uncheck=function(nodeId) {
	...
	for (var i = 0; i < children.length; i++) {
		...
		var idx = child.idx;
		if(document.getElementById("c" + this.obj + idx).checked ==  true){
			return;
		}
	}
	
	var parentNodeId = parentNode.idx;
	
	document.getElementById("c"+this.obj+parentNodeId).checked = false;
	this.uncheck(parentNodeId);
}
...
dTree.prototype.cc=function(nodeId){
	...
	while (parentNode != null) {
		document.getElementById("c" + this.obj + parentNode.idx).checked = cs;
...
// select children.
dTree.prototype.sc=function(nodeId) {
	var cs = document.getElementById("c"+this.obj+nodeId).checked;
	...
	for (var i = 0; i < children.length; i++) {
		var child = children[i];
		var idx = child.idx;
		document.getElementById("c" + this.obj + idx).checked = cs;
		this.sc(idx);
	}
}
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.check==true){
		str+= '<input type="checkbox" class="cx" id="c'+  this.obj + nodeId + '" name="' + node.checkboxname + '" value="' + node.checkboxvalue + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
	}
	if (node.url) {
