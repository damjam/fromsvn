...
public class GoogleHtmlNavigator implements HtmlNavigator {
	...
	public String getHtmlNavigation(String contextPath, Paginater p, int formIndex, boolean tidy) {
		...
		if (tidy) {
			html.append("<a href=\"javascript: Paginater.toPage(" + formIndex + "," + p.getCurrentPage() + ")\" id=\"_refresh\" style=\"display:none\">[刷新]</a>");
			html.append(getBatchPages(p, formIndex));
		}
		else {
			html.append("<a href=\"javascript: Paginater.toPage(" + formIndex + ","  + p.getCurrentPage() + ")\" id=\"_refresh\">[刷新]</a>");
	...
	private StringBuffer getBatchPages(Paginater p, int formIndex) {
		...
		for (Iterator i = batchPages.iterator(); i.hasNext();) {
			...
			if (p.getCurrentPage() == Integer.parseInt(page)) {
				...
				pages.append(" <a href=\"javascript: Paginater.toPage(" + formIndex + ","  + page + ")\">" + page + "</a> ");
			}
		}
