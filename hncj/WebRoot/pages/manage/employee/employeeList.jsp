...
<html lang="zh-cn"> 
	<head>
		...
		<script type="text/javascript">
			...
			function toChangeState(id, toState){
				if(window.confirm("确认操作?")){
					gotoUrl('${uri}?action=chagenState&id='+id+'&toState='+toState);
				}
			}
			function toApplyVoc(id){
				layer.prompt({title: '休假详情', formType: 2}, function(text){
					if(text.trim() == ''){
						alert('请输入休假详情(原因/起止日期等)');
						return;
					}
			        $.post(CONTEXT_PATH+'${uri}?action=addVocation&id='+id+'&transferDetail='+text, function(data){
			        	var json = eval('('+data+')');
			        	if(json.status == '1'){
			        		layer.msg('处理完成',{icon:1,shade:0.2,time:1000});
			        		gotoUrl('${url}?action=list&id='+id);
			        	}else{
			        		layer.msg('处理失败',{icon:0,shade:0.2,time:1000});
			        	}
			        });
			    });
			}
							...
							<tr align="center">
							    ...
							    <td class="redlink">
						    		...
						    		<c:if test="${sessionScope.branchNo eq '0000'}">
						    			<a href="javascript:toTransfer('${element.id}')" >部门/岗位变动</a>
						    			<a href="javascript:toChangeState('${element.id}', '02')" >离职</a>
						    		</c:if>
						    		<c:if test="${element.state eq '00' }">
						    			<a href="javascript:toApplyVoc('${element.id}')" >休假</a>
						    		</c:if>
						    		<c:if test="${element.state eq '01' }">
						    			<a href="javascript:toChangeState('${element.id}', '00')" >复职</a>
						    		</c:if>
