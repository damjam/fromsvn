...
public abstract class BaseDaoHibernateImpl extends RootDao implements BaseDao {
	...
	protected Paginater getPageData(final QueryHelper helper, final long pageNumber, final int pageSize) {
		return (Paginater) this.getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				List list = helper.getQuery(session).setMaxResults(paginater.getPageSize())
						.setFirstResult((int) paginater.getOffsetIndex()).list();
	...
	protected Paginater getPageDataBySql(final QueryHelper helper, final Paginater paginater) {
		return (Paginater) this.getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				List list = helper.getSQLQuery(session).setMaxResults(paginater.getPageSize())
						.setFirstResult((int) paginater.getOffsetIndex()).list();
				paginater.setData(list);
	...
	protected Paginater getPageDataBySql(final QueryHelper helper, final Pager pager) {
		return (Paginater) this.getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				Paginater paginater = new Paginater(helper.getRecordCountBySql(session), pager.getPageNumber(), pager
						.getPageSize());
	...
	protected Paginater getPageDataBySql(final QueryHelper helper, final Pager pager,
		...
		return (Paginater) this.getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				List list = helper.getSQLQuery(session).setMaxResults(paginater.getPageSize())
						.setFirstResult((int) paginater.getOffsetIndex()).setResultTransformer(transformer).list();
				paginater.setData(list);
