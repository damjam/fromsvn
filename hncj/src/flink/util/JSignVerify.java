...
public class JSignVerify {
	...
	public boolean VerifyFile(String inFile, byte[] pSigBuf , int SigLen) throws  Exception
	{
		byte[] buf=new byte[1024]; 
		int num;
		FileInputStream fin=new FileInputStream(inFile);
	...
	//验证签名
	public boolean VerifyMemoryMD5KMPS(byte[] info,byte[] signed) throws  Exception
	{
			java.security.Signature signetcheck=java.security.Signature.getInstance("MD5withRSA");
			signetcheck.initVerify(pubKey);
			ByteArrayOutputStream bs = new ByteArrayOutputStream();
	...
	public boolean VerifyFileKMPS(String inFile, byte[] pSigBuf , int SigLen) throws  Exception
	{
		byte[] buf=new byte[1024];
		int num;
		FileInputStream fin=new FileInputStream(inFile);
