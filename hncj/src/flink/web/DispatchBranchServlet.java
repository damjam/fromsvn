...
public class DispatchBranchServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		UserInfo userInfo = (UserInfo) WebUtils.getSessionAttribute(req,
				Constants.SESSION_USER);
		if (userInfo != null) {
			String sessionTag = (String) WebUtils.getSessionAttribute(req,
					Constants.BRANCH_TAG);
			if (!StringUtils.isEmpty(sessionTag)
					&& !StringUtils.equals(tag, sessionTag)) {
				req.setAttribute("tip",
						"您当前登录的机构为" + BranchType.getByTag(sessionTag).getName()
								+ ",如果您需要登录其他机构，请退出系统后重新登录");
			}
		...
		WebUtils.setSessionAttribute(req, Constants.BRANCH_TYPE,
				BranchType.getByTag(tag));
