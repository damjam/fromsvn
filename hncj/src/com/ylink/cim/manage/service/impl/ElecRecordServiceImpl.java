...
public class ElecRecordServiceImpl implements ElecRecordService{
	...
	public void saveElecRecord(ElecRecord elecRecord, UserInfo userInfo) throws BizException{
		...
		Assert.notNull(elecRecordDao.findById(HouseInfo.class, houseSn), "房屋信息不存在");
		...
		Assert.isEmpty(elecRecordDao.findPreRecords(params), "月份"+recordMonth+"的记录已存在");
			...
			if (account != null) {
				if(account.getBalance() > 0){
					...
					if (account.getBalance() >= elecBill.getAmount()) {
						...
						remark = "扣除水费"+MoneyUtil.getFormatStr2(elecBill.getAmount())+"元，当前余额"+MoneyUtil.getFormatStr2(balance)+"元";
						elecBill.setRemark("水费从预存账户中扣除，当前余额"+MoneyUtil.getFormatStr2(balance)+"元");
						...
						elecBill.setChargeDate(DateUtil.getCurrent());
						elecBill.setChargeUser(userInfo.getUserName());
						elecBill.setRemark("欠费"+MoneyUtil.getFormatStr2(elecBill.getAmount()-elecBill.getPaidAmt()));
