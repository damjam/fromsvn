...
public class PrivilegeAction extends BaseAction implements ModelDriven<Privilege>{
	...
	public String queryPopUpPrivilege() throws Exception {
		Paginater paginater = this.privilegeDao.getPrivilegePageList(model, getPager(request));
	...
	public String toEditPrivilege() throws Exception {
		if (StringUtils.isNotEmpty(model.getLimitId())) {
			//Privilege privilege = privilegeDao.findById(Privilege.class, model.getLimitId());
	...
	public String editPrivilege() throws Exception {
		try {
			Privilege p = privilegeDao.findById(Privilege.class, model.getLimitId());
	...
	public String delPri() throws Exception {
		try {
			...
			setResult(true, "操作成功", request);
			String msg = LogUtils.r("删除子权限成功,删除的内容为：{?}",FeildUtils.toString(model));
