...
public class DTree extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		if (null == data) {
			...
			imageLoading.append(" document.getElementById('" + this.getDisplayDivId() + "').innerHTML=LOAD_IMAGE;");
			imageLoading.append("</script>");
			out.print(imageLoading);

			// 样式加载
			StringBuffer styleLoading = new StringBuffer();
			styleLoading.append("<link rel='stylesheet' type='text/css' href=" + contextPath + this.getCssSrc() + "/>");

			StringBuffer sb = new StringBuffer();
			sb.append(" <script src='" + contextPath + this.getJsSrc() + "' type='text/javascript' ></script> ");
			sb.append(" <script type='text/javascript'>");
			sb.append(" var d = new dTree('d','" + contextPath + this.getImgSrc() + "');");
			initDtreeConfig(sb);
	...
	private void initDtree(StringBuffer sb, HttpServletRequest request) {
		...
		String initCheckIds = (String) request.getAttribute(this.initCheckDataName);
	...
	private void createBufferFunction(StringBuffer sb, HttpServletRequest request) {

		sb.append("function buf(){");
		sb.append("document.getElementById('" + this.displayDivId + "').innerHTML=d;");
