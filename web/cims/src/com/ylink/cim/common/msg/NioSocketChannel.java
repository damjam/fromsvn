...
public class NioSocketChannel
{
     ...
     * @throws Exception 
     */
    public void SendStringMsg(String v_sMsg)
        throws Exception
    {
        if (v_sMsg == null || v_sMsg.length() <= 0 || m_Socket == null)
            return;
        v_sMsg =
            CommUtil.FILL(String.valueOf(v_sMsg.toString().length()), '0', Constant.MSG_LEN_LEN, 'L')
                + v_sMsg.toString();
        //v_sMsg  = CommUtil.FillHeadLength(v_sMsg.toString(),'0',Constant.MSG_LEN_LEN, 'L');
     ...
     * @throws Exception 
     */
    public void SendBufferMsg(StringBuffer v_sMsg)
        throws Exception
    {
        ...
        throws Exception
    {
        ...
        if (this.bIsAutoUnZipByReqMsg)
        {
            ...
            if (this.mEncryptMode == SSLConstant.ENCRYPT_MODE_DEFAULT_ZIP)
            {
                ...
                zipBuff =
                    ByteBuffer.wrap(DESede.encryptSrcMsg(this.mHeadLen, this.mEncryptMode, this.mSessionId, zipBytes));
            }
            else
            {
                byte[] zipBytes = this.zipWriteBytes(writeBuff.array());
     ...
     * @throws Exception
     */
    public synchronized String doReadBufferMsg()
        throws Exception
    {
        if (m_Socket == null)
            return "";
