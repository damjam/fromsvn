...
public class BillTrackAction extends BaseDispatchAction {
	...
	public ActionForward generateAll(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		try {
				Integer cnt = billTrackService.generateTrack();
				setResult(true, "操作成功,共生成"+cnt+"条", request);
			}catch (BizException e) {
				e.printStackTrace();
				setResult(false, e.getMessage(), request);
			} catch (Exception e) {
