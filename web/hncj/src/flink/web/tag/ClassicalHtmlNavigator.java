	...
	public String getHtmlNavigation(String contextPath, Paginater p, int formIndex, boolean tidy) {
		...
		if (tidy) {
			...
			if (p.getCurrentPage() != p.getFirstPage()) {
				html.append("<a href=\"javascript: Paginater.toPage(" + formIndex + "," + p.getFirstPage()
						+ ")\">[首页]</a>");
			...
			if (p.getCurrentPage() != p.getLastPage()) {
				html.append("<a href=\"javascript: Paginater.toPage(" + formIndex + "," + p.getNextPage()
						+ ")\">[下一页]</a>");
				html.append("<a href=\"javascript: Paginater.toPage(" + formIndex + "," + p.getLastPage()
						+ ")\">[末页]</a>");
			}
		} else {
		...
		html.append("&nbsp;&nbsp;转至第<input type=\"text\" id=\"goPageIndex_" + formIndex
				+ "\" size=\"3\" class=\"pageinputtext\" />页");
		html.append("<img src=\"" + contextPath + "/images/jumpto.bmp\" onclick=\"Paginater.goPage(" + formIndex
				+ ")\" style=\"cursor: hand\" />");
