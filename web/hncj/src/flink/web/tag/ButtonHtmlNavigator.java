...
public class ButtonHtmlNavigator implements HtmlNavigator {
	...
	public String getHtmlNavigation(String contextPath, Paginater p, int formIndex, boolean tidy) {
		...
		html.append("  <span>");
		html.append("    <input type='text' name='pageSize' id='pageSize_" + formIndex + "' class='pageinputtext' value='" + p.getPageSize() + "' onkeypress='Paginater.keyToPage(" + formIndex + ")'/>条记录／页");
		html.append("    当前第<input type='text' name='pageNumber' id='goPageIndex_" + formIndex + "' class='pageinputtext' value='" + p.getCurrentPage() + "' onkeypress='Paginater.keyToPage(" + formIndex + ")' />页");
		html.append("  </span>");
