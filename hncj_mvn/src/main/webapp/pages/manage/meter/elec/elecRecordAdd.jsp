...
<script type="text/javascript">
	...
	function getPreRecord() {
		...
		$.post(CONTEXT_PATH + '/${uri}?action=getPreRecord&houseSn=' + houseSn,
				function(data) {
					if (data != null) {
			...
			<div class="widget">
				<table class="form_grid">
					...
					<tr>
						...
						<td><s:textfield name="houseSn" id="houseSn"
								class="{required:true}" maxlength="10" onblur="getPreRecord()" />
							<span class="field_tipinfo">不能为空</span></td>
					</tr>
					<tr>
						<td class="formlabel nes">用电月份</td>
						<td><s:textfield name="recordMonth" id="recordMonth"
								class="{required:true}" /> <span class="field_tipinfo">不能为空</span>
						</td>
					</tr>
					<tr>
						<td class="formlabel nes">上期抄表日期</td>
						<td><s:textfield name="preRecordDate" id="preRecordDate"
								class="{required:true,digit:true}" maxlength="8"
								onfocus="WdatePicker();" readonly="true" /> <span
							class="field_tipinfo">不能为空</span></td>
					</tr>
					<!--  -->
					<tr>
						<td class="formlabel nes">上期读数</td>
						<td><s:textfield name="prenum" id="prenum"
								class="{required:true,digit:true}" maxlength="8" /> <span
							class="field_tipinfo">请输入正确的数字</span></td>
					</tr>
					<tr>
						<td class="formlabel nes">本期抄表日期</td>
						<td><s:textfield name="curRecordDate" id="curRecordDate"
								class="{required:true}" maxlength="8" readonly="true"
								onfocus="WdatePicker();" /> <span class="field_tipinfo">不能为空</span>
						</td>
					</tr>
					<tr>
						<td class="formlabel nes">本期读数</td>
						<td><s:textfield name="curnum" id="curnum"
								class="{required:true,digit:true}" maxlength="8"
								onblur="checkNum()" /> <span class="field_tipinfo">请输入正确的数字</span>
						</td>
					</tr>
					<tr>
						<td class="formlabel nes">实际用量</td>
						<td><s:textfield name="num" id="num" class="{required:true}"
								maxlength="25" readonly="true" /> <span class="field_tipinfo">不能为空</span>
						</td>
					</tr>
				</table>
				<div class="btnbox">
