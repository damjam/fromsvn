...
public class NoticeMsgDaoImpl extends BaseDaoImpl implements NoticeMsgDao {
	...
	public Paginater findPaginater(Map<String, Object> map, Pager pager) {
		...
		if (map.get("startCreateDate") != null) {
			helper.append("and t.createTime >= ?", DateUtil.string2Date(
					MapUtils.getString(map, "startCreateDate"), "yyyyMMdd"));
		}
		if (map.get("endCreateDate") != null) {
			helper.append("and t.createTime <= ?", DateUtil.string2Date(
					MapUtils.getString(map, "endCreateDate"), "yyyyMMdd"));
		}
		if (!BranchType.HQ_0000.getValue().equals(map.get("branchNo"))) {
			helper.append("and t.branchNo = ?", map.get("branchNo"));
		}
		helper.append("and t.subject like ?", (String) map.get("subject"),
				MatchMode.ANYWHERE);
